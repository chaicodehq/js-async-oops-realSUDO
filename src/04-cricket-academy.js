...
export class Batsman extends Player {
	...
	playInnings(runs, balls) {
		...
		const obj = { runs, balls, strikeRate: (runs / balls) * 100 };
		this.innings.push(obj);
		return obj;
	}
	getStrikeRate() {
		if (!this.innings.length) return 0;
		return (
			this.innings.reduce((s, i) => s + i.strikeRate, 0) / this.innings.length
		);
	}
	getProfile() {
		return {
			...
			role: "batsman",
			totalRuns: this.innings.reduce((s, i) => s + i.runs, 0),
			inningsPlayed: this.innings.length,
		};
	}
}
...
export class Bowler extends Player {
	...
	bowlSpell(wickets, runsConceded, overs) {
		...
		const obj = { wickets, runsConceded, overs, economy: runsConceded / overs };
		this.spells.push(obj);
		return obj;
	}
	getEconomy() {
		if (!this.spells.length) return 0;
		return (
			this.spells.reduce((s, sp) => s + sp.economy, 0) / this.spells.length
		);
	}
	getProfile() {
		return {
			...
			role: "bowler",
			totalWickets: this.spells.reduce((s, sp) => s + sp.wickets, 0),
			spellsBowled: this.spells.length,
		};
	}
}
