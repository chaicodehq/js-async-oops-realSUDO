...
export class Incubator {
	...
	async admitStartup(startup) {
		...
		if (this.#startups.find((s) => s.startup.name === startup.name))
	...
	async assignMentor(startupName, mentor) {
		await new Promise((r) => setTimeout(r, 50));
		if (!this.#startups.find((s) => s.startup.name === startupName))
	...
	async conductDemo(startupName) {
		...
		return {
			...
			feedback: feedbacks[Math.floor(Math.random() * 3)],
			timestamp: new Date().toISOString(),
		};
	}

	async batchProcess(startups) {
		return Promise.allSettled(startups.map((s) => this.admitStartup(s)));
	}
	...
	getTopFunded(n) {
		if (n <= 0 || !this.#startups.length) return [];
		return [...this.#startups]
	...
	*[Symbol.iterator]() {
		for (const s of this.#startups) yield s.startup;
	}
